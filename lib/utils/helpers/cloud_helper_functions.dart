...
class PCloudHelperFunctions {
  ...
  static Widget? checkSingleRecordState<P>(AsyncSnapshot<P> snapshot) {
  ...
  /// If an error occurs, it returns a generic error message.
  /// Otherwise, it returns null.
  static Widget? checkMultiRecordState<P>({required AsyncSnapshot<List<P>> snapshot, Widget? loader, Widget? error, Widget? nothingFound}) {
    if (snapshot.connectionState == ConnectionState.waiting) {
