...
class StaffRows extends DataTableSource {
  ...
  DataRow? getRow(int index) {
    ...
    return DataRow2(
      onTap: () => Get.toNamed(PRoutes.editStaff, arguments: customer, parameters: {'customerId': customer.id ?? ''}),
      cells: [
        DataCell(
            Row(
              children: [
        ...
        DataCell(
            PTableActionButtons(
              view: false,
              edit: true,
              onEditPressed: () => Get.toNamed(PRoutes.editStaff, arguments: customer, parameters: {'customerId': customer.id ?? ''}),
