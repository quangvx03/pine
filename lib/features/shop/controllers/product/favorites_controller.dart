...
class FavoritesController extends GetxController {
  ...
  void toggleFavoriteProduct(String productId) {
    if (!favorites.containsKey(productId)) {
      ...
      PLoaders.successOBar(message: 'Đã thêm sản phẩm vào Yêu thích.');
    } else {
      PLocalStorage.instance().removeData(productId);
      favorites.remove(productId);
      saveFavoritesToStorage();
      favorites.refresh();
      PLoaders.successOBar(message: 'Đã xoá sản phẩm khỏi Yêu thích.');
    }
  }

  void saveFavoritesToStorage() {
    final encodedFavorites = json.encode(favorites);
    PLocalStorage.instance().writeData('favorites', encodedFavorites);
  }
